...
def main():
    ...
    # read through idx segment chunks
    idx_files_listed = 0
    idx_files_scanned = 0
    idx_cf_events = 0
    idx_layout_version = 'idx/segments'

    print('Scanning idx segment manifests for chunkFilePaths...')
    for date_prefix in iter_changefeed_date_prefixes(idx_layout_version, start_hour_utc, end_hour_utc):
        print(f'Checking idx prefix: {date_prefix}')
        for segment_blob in container_client.list_blobs(name_starts_with=date_prefix):
            if not segment_blob.name.endswith('/meta.json'):
                continue
            idx_files_listed += 1

            meta_client = container_client.get_blob_client(segment_blob.name)
            meta_bytes = meta_client.download_blob().readall()
            meta = json.loads(meta_bytes)

            segment_begin_utc = datetime.fromisoformat(meta['begin'].replace('Z', '+00:00'))
            segment_hour_utc = segment_begin_utc.replace(minute=0, second=0, microsecond=0)
            if not (start_hour_utc <= segment_hour_utc <= end_hour_utc):
                continue

            for chunk_path in meta.get('chunkFilePaths', []):
                chunk_prefix = chunk_path
                if chunk_prefix.startswith('$blobchangefeed/'):
                    chunk_prefix = chunk_prefix[len('$blobchangefeed/'):]
                chunk_prefix = chunk_prefix.lstrip('/')

                for chunk_blob in container_client.list_blobs(name_starts_with=chunk_prefix):
                    if not chunk_blob.name.endswith('.avro'):
                        continue
                    idx_files_scanned += 1

                    chunk_blob_client = container_client.get_blob_client(chunk_blob.name)
                    chunk_avro_bytes = chunk_blob_client.download_blob().readall()
                    chunk_avro_stream = BytesIO(chunk_avro_bytes)
                    chunk_reader = datafile.DataFileReader(chunk_avro_stream, io.DatumReader())
    ...
    # the $changefeed has two folders that could represent the changefeed events
    # testing to see /log vs. /idx event scanning!

    # /log
    print()
    print(f'Blobs listed: {log_files_listed}')
    print(f'Files scanned: {log_files_scanned}')
    print(f'Total events: {log_cf_events}')

    # /idx
    print()
    print(f'Blobs listed: {idx_files_listed}')
    print(f'Files scanned: {idx_files_scanned}')
    print(f'Total events: {idx_cf_events}')
