...
class AsyncCharacterEmbeddings(CharacterEmbeddings):
    """CharacterEmbeddings with a distinct async path that tracks calls.

    This subclass overrides ``aembed_documents`` so that tests can verify
    the async store actually calls the async method rather than the sync one.
    """

    def __init__(self, dims: int = 50, seed: int = 42):
        super().__init__(dims=dims, seed=seed)
        self.aembed_calls: int = 0
        self.sync_embed_calls: int = 0
